...
import { checkKieConnection, isKieConnected } from '../utils/kieAuth'
  ...
  return (
    <div style={{ paddingTop: 'var(--nav-h)', minHeight: '100vh', background: 'var(--bg)' }}>
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <Section title="KIE.AI Engine">
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16, lineHeight: 1.6 }}>
            The application uses the company's KIE.AI API key configured server-side. Users do not need to log in or connect their own accounts.
          </p>
          {checking ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid rgba(139,92,246,0.3)', borderTopColor: '#8B5CF6', animation: 'spin 0.7s linear infinite' }} />
              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Checking KIE.AI connection status…</span>
            </div>
          ) : kieConnected ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#34C759' }} />
              <span style={{ fontSize: 13, fontWeight: 600, color: '#34C759' }}>Engine ready — using company credits</span>
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#FF3B30' }} />
              <span style={{ fontSize: 13, fontWeight: 600, color: '#FF3B30' }}>Engine offline — KIE_API_KEY missing or invalid in server .env</span>
            </div>
          )}
